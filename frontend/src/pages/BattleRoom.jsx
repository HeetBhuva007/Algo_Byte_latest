...
//       // Sync Redux state when local theme changes
//       useEffect(() => {
...
export default function BattleRoom() {
    ...
    // MODIFICATION: Use Redux state for theme
    const { isDark, user } = useSelector((state) => state.auth);
    const dispatch = useDispatch();

    // MODIFICATION: Remove local theme state: `const [theme, setTheme] = useState('light');`
    // MODIFICATION: Remove useEffects that sync local state with Redux.

    // This useEffect correctly syncs the global Redux state to the DOM
    useEffect(() => {
        document.documentElement.classList.toggle('dark', isDark);
    }, [isDark]);

    // MODIFICATION: The toggle function is now handled directly in the JSX
    // `const toggleTheme = () => setTheme(prev => (prev === 'light' ? 'dark' : 'light'));` is removed.
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-black text-slate-800 dark:text-slate-200 flex items-center justify-center p-4 transition-colors duration-300 font-mono">
            {/* MODIFICATION: Pass `isDark` directly from Redux */}
            <FloatingCodeBackground isDark={isDark} />
            
            {/* MODIFICATION: Pass `isDark` and dispatch an action to toggle the theme */}
            <ThemeToggle isDark={isDark} toggleTheme={() => dispatch(setMode(!isDark))} />
