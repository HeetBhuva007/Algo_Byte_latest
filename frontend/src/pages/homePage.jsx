...
// import { allProblemsFetch, logoutUser, getUserImage, setMode } from "../authSlice"
...
function Homepage() {
  const dispatch = useDispatch()
  // MODIFICATION: Select `isDark` from the Redux store
  const { user, userImage, isDark } = useSelector((state) => state.auth)
  const problems = useSelector((state) => state.auth.problemsBySlice)
  const [solvedProblems, setSolvedProblems] = useState([])
  const [userStreak, setUserStreak] = useState(null)
  const [problemOfTheDay, setProblemOfTheDay] = useState(null)
  ...
  const [searchTerm, setSearchTerm] = useState("")
  const [showAdvancedFilters, setShowAdvancedFilters] = useState(false)
  // MODIFICATION: Removed local state `const [isDark, setIsDark] = useState(false)`
  const [isMenuOpen, setIsMenuOpen] = useState(false)
  const [loading, setLoading] = useState(true)
  const [selectedBookmark, setSelectedBookmark] = useState(null)
  const [selectedTag, setSelectedTag] = useState(null)
  ...
  // MODIFICATION: Removed the useEffect that was dispatching the local state
  // This is no longer necessary as we read directly from and dispatch directly to Redux.
      ...
      >
        <div className="max-w-[1600px] mx-auto flex items-center justify-between px-6 py-3">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* MODIFICATION: Dispatch the `setMode` action on click */}
            <button
              onClick={() => dispatch(setMode(!isDark))}
              className={`p-2 rounded-lg ${isDark ? "bg-slate-800/50 text-slate-300 hover:bg-slate-800" : "bg-white text-slate-600 hover:bg-slate-100"} border border-transparent hover:border-slate-300 dark:hover:border-slate-700 transition-all duration-300 group`}
              title="Toggle theme"
            >
              {isDark ? (
